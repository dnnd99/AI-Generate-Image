...
// Helper to instantiate GenAI server-side
...
function generateFallbackIconCanvas(prompt: string, style: string, layout: string, resolution: string): string {
  // We return SVG data URI that renders high quality vector graphics
  const size = resolution === "4K" ? 2048 : resolution === "2K" ? 1024 : 512;
  const isGrid = layout === "grid";

  const styleColors: Record<string, { bg: string; stroke: string; fill: string; fill2: string; accent: string }> = {
    ...
    duotone: { bg: "#ffffff", stroke: "#059669", fill: "rgba(16, 185, 129, 0.2)", fill2: "#047857", accent: "#10b981" },
    glossy_jelly: { bg: "#ffffff", stroke: "none", fill: "url(#jellyGrad)", fill2: "url(#jellyGrad2)", accent: "#34d399" },
  };

  const palette = styleColors[style] || styleColors.flat;

  // Icon geometry based on prompt keywords
  let pathShape = "";
  ...
  if (lower.includes("cart") || lower.includes("shopping") || lower.includes("belanja") || lower.includes("keranjang")) {
    ...
    // Default modern abstract symbol
...
// Route: API Icon Generator using Gemini gemini-3.1-flash-image or fallback
app.post("/api/generate-icon", async (req, res) => {
  try {
    const { prompt, style, layout, resolution } = req.body;
    ...
    // Map style names to precise vector microstock visual style prompts
    ...
    const layoutPrompt = layout === "grid" 
    ...
    if (ai) {
      try {
        // Attempt using gemini-3.1-flash-image
    ...
    // Fallback vector icon generation if Gemini API key isn't configured or model fails
    ...
    return res.json({
      ...
      fullPrompt,
      mode: "fallback",
      notice: !ai ? "Operating in local vector engine mode. Set GEMINI_API_KEY in Secrets for photorealistic Gemini 3.1 AI generations." : undefined,
    });

  } catch (error: any) {
    console.error("Error generating icon:", error);
    res.status(500).json({ error: error?.message || "Failed to generate icon" });
  }
});

// Route: Generate Microstock Metadata (Title, Keywords/Tags, Category) for generated icons
...
- "tags": An array of 25 to 35 highly relevant SEO keywords/tags for stock search indexing.`;
    ...
    return res.json({
      title: `${req.body.prompt || "Vector"} Icon Asset`,
      category: "Vectors",
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`Icon Studio AI Server running at http://0.0.0.0:${PORT}`);
  });
}
